...
// infrastructure/release-custody/owner-gate/ledger-connection.mjs
var requireThat = (value, message) => {
  if (!value) throw new Error(message);
};
async function connectProtectedLedger({ connectionString, policy, clientFactory = (config) => new esm_default.Client(config) }) {
  const url = new URL(connectionString);
  requireThat(["postgres:", "postgresql:"].includes(url.protocol) && /^[a-z0-9.-]+\.neon\.tech$/.test(policy.hostname) && url.hostname === policy.hostname && (!url.port || url.port === "5432") && decodeURIComponent(url.pathname.slice(1)) === policy.database && decodeURIComponent(url.username) === policy.role && policy.role === "cadenza_owner_release_runtime" && url.password.length > 0, "Wrong protected ledger connection");
  const client = clientFactory({
    ...
    query_timeout: 2e4,
    application_name: "cadenza-protected-release-ledger"
  });
  try {
    ...
    const { rows } = await client.query(`SELECT current_database() AS database, current_user AS role,
      ...
      has_schema_privilege(current_user, 'release_gate', 'CREATE') AS can_create,
      EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
        WHERE n.nspname='release_gate' AND c.relkind IN ('r','p')
        AND has_table_privilege(current_user,c.oid,'INSERT,UPDATE,DELETE,TRUNCATE')) AS can_write_tables
      FROM pg_roles r WHERE r.rolname=current_user`);
    const row = rows[0];
    requireThat(rows.length === 1 && row.database === policy.database && row.role === policy.role && ["rolsuper", "rolcreatedb", "rolcreaterole", "rolbypassrls", "can_create", "can_write_tables"].every((key) => row[key] === false), "Ledger runtime is overprivileged or unidentified");
    return { query: (sql, values) => client.query(sql, values), close: () => client.end() };
  } catch (error) {
